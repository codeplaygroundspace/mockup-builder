...
## 1. Start by Reading Project Context

- **Before making changes or running project-specific commands, read `AGENTS.md` and
  `README.md`.** This applies to all AI coding assistants, including Codex, GPT, Claude,
  and similar tools.
- Treat `AGENTS.md` as the operational rules for agent work and `README.md` as the
  current project overview, setup notes, and backlog context.
- If either file conflicts with a direct user instruction, follow the user's latest
  instruction and mention the conflict briefly.

## 2. Use the Development Server, **not** `npm run build`
